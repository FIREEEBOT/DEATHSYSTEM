...
async def scan(event, flags):
    ...
    await event.reply("Connecting to Cyber Security System (CSS) for an Internet Scan")
...
async def revive(event):
    ...
    if not (
        ...
        return
    await a.edit("Revert request sent to Cyber. This might take 5minutes or so.")
